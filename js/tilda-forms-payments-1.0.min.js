...
function t_triggerEvent(t, e) {
    var o;
    document.createEvent ? (o = document.createEvent("HTMLEvents")).initEvent(e, !0, !1) : document.createEventObject && ((o = document.createEventObject()).eventType = e), o.eventName = e, t.dispatchEvent ? t.dispatchEvent(o) : t.fireEvent ? t.fireEvent("on" + o.eventType, o) : t[e] ? t[e]() : t["on" + e] && t["on" + e]()
}
window.tildaForm.addPaymentInfoToForm = function(t) {
    ...
    if (!window.tcart.delivery && s && parseInt(s.getAttribute("data-delivery-price"), 10) > 0) try {
    ...
    var m = {},
        w = "",
        p = 0;
    window.tcart.products && window.tcart.products.length > 0 && (p = window.tcart.products.length), window.tildaForm.tildapayment.products = [];
    for (var y = 0; y < p; y++) {
        var a = window.tcart.products[y];
        for (var f in m = {}, w = "", window.tildaForm.tildapayment.products[y] = {}, a)
            if ("function" != typeof a[f])
                if ("options" === f)
                    for (var v in window.tildaForm.tildapayment.products[y][f] = {}, a[f]) window.tildaForm.tildapayment.products[y][f][v] || (window.tildaForm.tildapayment.products[y][f][v] = {}), a[f][v].option && (window.tildaForm.tildapayment.products[y][f][v].option = a[f][v].option), a[f][v].price && a[f][v].price > 0 && (window.tildaForm.tildapayment.products[y][f][v].price = a[f][v].price), a[f][v].variant && (window.tildaForm.tildapayment.products[y][f][v].variant = a[f][v].variant), a[f][v].option && a[f][v].variant && (w && (w += ", "), w = w + a[f][v].option + ":" + a[f][v].variant);
                else window.tildaForm.tildapayment.products[y][f] = a[f];
        a.sku ? m.id = a.sku : a.uid && (m.id = a.uid), m.name = a.name, a.price ? (m.price = a.price, m.quantity = parseInt(a.amount / a.price)) : a.quantity && a.quantity > 1 ? (m.price = a.amount / a.quantity, m.quantity = a.quantity) : (m.price = a.amount, m.quantity = 1), m.name = a.name, w && (m.variant = w), a.sku && (m.sku = a.sku), a.uid && (m.uid = a.uid), window.tildaForm.arProductsForStat.push(m)
    }
    var g = 0;
    window.tcart.delivery && window.tcart.delivery.name && (window.tildaForm.tildapayment.delivery = {
        name: window.tcart.delivery.name
    }, window.tcart.delivery && window.tcart.delivery.price >= 0 && (g = window.tcart.delivery.price, window.tildaForm.tildapayment.delivery.price = window.tcart.delivery.price, window.tcart.prodamount > 0 && window.tcart.delivery.freedl && window.tcart.delivery.freedl > 0 && (window.tildaForm.tildapayment.delivery.freedl = window.tcart.delivery.freedl, window.tcart.prodamount - r >= window.tcart.delivery.freedl && (g = 0)), m = {
        name: window.tcart.delivery.name,
        price: g,
        quantity: 1,
        id: "delivery"
    }, window.tildaForm.arProductsForStat.push(m)));
    try {
        var b = ["city", "street", "pickup-name", "pickup-address", "pickup-id", "house", "entrance", "floor", "aptoffice", "phone", "entrancecode", "comment", "service-id", "hash", "postalcode", "country", "userinitials", "onelineaddress", "lang"];
    ...
    if ("y" !== form.getAttribute("data-formpaymentoff")) {
        if (successBox) {
            ...
            successBoxText && (form.tildaSuccessMessage = successBoxText), t_forms__getMsg("successorder") && (successBox.innerHTML = t_forms__getMsg("successorder")), successBox.style.display = "block"
        }
        if (t_addClass(form, "js-send-form-success"), "link" === objNext.type) return window.tildaForm.clearTCart(form), objNext.message && successBox && (successBox.innerHTML = objNext.message, successBox.style.display = "block", t_onFuncLoad("t_forms__parseVariables", (function() {
            t_forms__parseVariables(successBox), t_forms__updateAllSystemVariables(form)
        }))), window.location.href = objNext.value, !0;
        ...
        if ("function" === objNext.type) {
            ...
            return objNext.value.functioncode ? window.tildaForm.paysystemRun(objNext.value.script, objNext.value.sysname, form, objNext.value.functioncode, arrArgs) : "undefined" != typeof jQuery ? eval(objNext.value.name + "($(form), arrArgs);") : eval(objNext.value.name + "(form, arrArgs);"), !1
        }
        window.tildaForm.clearTCart(form), "text" === objNext.type && objNext.message && successBox && (successBox.innerHTML = objNext.message, successBox.style.display = "block", t_onFuncLoad("t_forms__parseVariables", (function() {
            t_forms__parseVariables(successBox), t_forms__updateAllSystemVariables(form)
        })))
    ...
    if (window.scriptSysPayment || (window.scriptSysPayment = {}), !window.scriptSysPayment[e] || !0 !== window.scriptSysPayment[e]) {
        ...
        o.type = "text/javascript", o.src = t, document.body.appendChild(o), window.scriptSysPayment[e] = !0
    }
    ...
    var script = linkScript,
        ...
        objArgs = arArgs;
    if ("undefined" != typeof jQuery) var $jform = $(form);
    eval(functionCode)
}, window.tildaForm.paysystemSuccess = function(t, e) {
    t instanceof Element || (t = t[0]), window.tildaForm.clearTCart(t);
    var o = document.getElementById("allrecords"),
        ...
        s = e.amount,
        d = e.description;
    if (window.dataSuccessMessage = "", window.Tilda && "function" == typeof window.Tilda.sendEventToStatistics) {
        var c = o.getAttribute("data-tilda-currency") || "";
        if (!c) {
            var l = document.querySelector(".t706");
            c = l ? l.getAttribute("data-project-currency-code") : ""
        }
        c || o.setAttribute("data-tilda-currency", e.currency), window.Tilda.sendEventToStatistics(i, n, d, s)
    }
    t.tildaSuccessMessage && (window.dataSuccessMessage = t.tildaSuccessMessage), e.successurl && window.setTimeout((function() {
        window.location.href = e.successurl
    }), 300), r.innerHTML = window.dataSuccessMessage ? window.dataSuccessMessage : "", t.tildaSuccessMessage = "";
    var u = t.getAttribute("data-success-callback");
    window.tildaForm.successEnd(t, e.successurl, u), t_triggerEvent(t, "tildaform:aftersuccess")
    ...
    if (t instanceof Element || (t = t[0]), !0 !== window.stripeapiiscalled) return window.tildaForm.stripeLoad(), window.setTimeout((function() {
    ...
    var o = document.getElementById("allrecords"),
        a = e.companyname,
        r = e.companylogo;
    if (a || (a = window.location.host), !window.stripehandler) {
        ...
        e.zipCode && 1 === e.zipCode && (i.zipCode = !0), e.billingAddress && 1 === e.billingAddress && (i.billingAddress = !0), e.shipping && 1 === e.shipping && (i.shippingAddress = !0), window.stripehandler = window.StripeCheckout.configure(i), t_addEventListener(window, "popstate", (function() {
    ...
    window.stripehandler.open({
        ...
        token: function a(r) {
            if (r && r.id) {
                ...
                d.open("POST", "https://" + window.tildaForm.endpoint + "/payment/stripe/", !0), d.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8"), d.setRequestHeader("Accept", "application/json, text/javascript, */*; q=0.01"), d.onreadystatechange = function() {
                    if (4 === d.readyState && d.status >= 200 && d.status < 400) {
                        var a = d.responseText,
                            r;
                        if (a)
                            if ("object" == typeof JSON.parse(a)) {
                                window.tildaForm.clearTCart(t);
                                var i = "/tilda/" + s + "/payment/",
                                    n = "Pay order in form " + s,
                                    c = e.amount,
                                    l = e.description,
                                ...
                                if (window.Tilda && "function" == typeof window.Tilda.sendEventToStatistics) {
                                    ...
                                    o.getAttribute("data-tilda-currency") || m && m.getAttribute("data-project-currency-code") || o.setAttribute("data-tilda-currency", e.currency), window.Tilda.sendEventToStatistics(i, n, l, c)
                                }
                                t.tildaSuccessMessage && (window.dataSuccessMessage = t.tildaSuccessMessage), e.successurl && window.setTimeout((function() {
                                    window.location.href = e.successurl
                                }), 300), u.innerHTML = window.dataSuccessMessage ? window.dataSuccessMessage : "", t.tildaSuccessMessage = "";
                                var w = t.getAttribute("data-success-callback");
                                window.tildaForm.successEnd(t, e.successurl, w), t_triggerEvent(t, "tildaform:aftersuccess")
                            }
                    }
                }, d.send(i)
            }
    ...
    if (!window.cloudpaymentsapiiscalled) {
        var t = document.createElement("script");
        t.type = "text/javascript", t.src = "https://widget.cloudpayments.ru/bundles/cloudpayments.js", document.body.appendChild(t), window.cloudpaymentsapiiscalled = !0
    }
}, window.tildaForm.cloudpaymentPay = function(t, e) {
    if (t instanceof Element || (t = t[0]), !0 !== window.cloudpaymentsapiiscalled) return window.tildaForm.cloudpaymentLoad(), window.setTimeout((function() {
    ...
    var i = e.currency,
        n = e.language,
        s = {};
    if (n || (n = "RUB" == i || "BYR" == i || "BYN" == i || "RUR" == i ? "ru-RU" : "UAH" == i ? "uk" : "en-US"), !window.cloudpaymentshandler) {
        if ("object" != typeof window.cp) return window.setTimeout((function() {
            window.tildaForm.cloudpaymentPay(t, e)
        }), 200), !1;
        s = {
            language: n
        }, e.applePaySupport && "off" === e.applePaySupport && (s.applePaySupport = !1), e.googlePaySupport && "off" === e.googlePaySupport && (s.googlePaySupport = !1), window.cloudpaymentshandler = new cp.CloudPayments(s)
    }
    var d = {};
    d.projectid = e.projectid, e.cloudPayments && (e.cloudPayments.recurrent || e.cloudPayments.customerReceipt) && (d.cloudPayments = e.cloudPayments);
    var c = "",
        l = t.closest(".t-popup_show");
    return l || (l = t.closest(".t706__cartwin_showed")), l && (c = l.getAttribute("style"), l.style.zIndex = 100), window.tildaForm.orderIdForStat = e.invoiceId, e.skin || (e.skin = "classic"), window.cloudpaymentshandler.charge({
        publicId: e.publicId,
        description: e.description,
        ...
        window.cloudpaymentshandler = !1, l && c && l.setAttribute("style", c);
        var s = "/tilda/" + a + "/payment/",
            d = "Pay order in form " + a,
            u = e.amount,
            m = e.description;
        o.setAttribute("data-tilda-currency", i), window.Tilda && "function" == typeof window.Tilda.sendEventToStatistics && window.Tilda.sendEventToStatistics(s, d, m, u), window.tildaForm.clearTCart(t), e.successurl && window.setTimeout((function() {
            window.location.href = e.successurl
        }), 300), t.tildaSuccessMessage && (window.dataSuccessMessage = t.tildaSuccessMessage), r.innerHTML = window.dataSuccessMessage ? window.dataSuccessMessage : "", t.tildaSuccessMessage = "";
        var w = t.getAttribute("data-success-callback");
        window.tildaForm.successEnd(t, e.successurl, w), t_triggerEvent(t, "tildaform:aftersuccess")
    }), (function(o, a) {
        if (l && c && l.setAttribute("style", c), e.failureurl) window.location.href = e.failureurl;
        else {
            if (l) {
                ...
                i && (i.style.display = "block"), n && (n.style.display = "block"), s && (s.style.display = "block")
            }
        ...
        r.style.display = "none", t.tildaSuccessMessage && (window.dataSuccessMessage = t.tildaSuccessMessage), r.innerHTML = window.dataSuccessMessage ? window.dataSuccessMessage : "", t.tildaSuccessMessage = "", window.cloudpaymentshandler = !1
    })), !1
    ...
    if (window.dataSuccessMessage = "", sendStat) {
        ...
        if (window.Tilda && "function" == typeof window.Tilda.sendEventToStatistics) {
            ...
            allRecords.getAttribute("data-tilda-currency") || t706block && t706block.getAttribute("data-project-currency-code") || allRecords.setAttribute("data-tilda-currency", arArgs.currency), window.Tilda.sendEventToStatistics(linkPage, title, product, price)
        }
    }
    successBox && ("y" === dataSuccessPopup && (successBox.style.display = "none"), arArgs.successmessage ? successBox.innerHTML = arArgs.successmessage : form.tildaSuccessMessage ? successBox.innerHTML = form.tildaSuccessMessage : t_forms__getMsg("success") ? successBox.innerHTML = t_forms__getMsg("success") : successBox.innerHTML = "", form.tildaSuccessMessage = "", "y" === dataSuccessPopup ? window.tildaForm.showSuccessPopup(successBox.textContent || successBox.innerText) : successBox.style.display = "block"), t_addClass(form, "js-send-form-success"), window.tildaForm.clearTCart(form), arArgs.successurl && window.setTimeout((function() {
        window.location.href = arArgs.successurl
    }), 300);
    var successCallback = form.getAttribute("data-success-callback");
    successCallback && 0 === successCallback.indexOf("window.") && (successCallback = successCallback.split(".")[1]), successCallback && "function" == typeof window[successCallback] && ("undefined" != typeof jQuery ? eval(successCallback + "($(form))") : eval(successCallback + "(form)"));
    ...
    Array.prototype.forEach.call(upwidgetRemoveBtns, (function(t) {
        ...
        tranId: "0",
        orderId: "0"
    }), form.tildaTranId = "0", form.tildaOrderId = "0", t_triggerEvent(form, "tildaform:aftersuccess")
}, window.tildaForm.banktransferPay = function(t, e) {
    if (t instanceof Element || (t = t[0]), e && "fast" === e.condition) window.tildaForm.sendStatAndShowMessage(t, e, !0);
    else if (e && e.html) {
        var o = document.body,
            a;
        document.getElementById("allrecords").insertAdjacentHTML("beforeend", '<div id="banktransfer">' + e.html + "</div>");
        var r = document.getElementById("banktransfer"),
            i, n = document.querySelector(".t-banktransfer").querySelector(".t-popup__close");

        function s() {
            t_removeClass(o, "t-body_popupshowed"), r && t_removeEl(r);
            try {
                "function" == typeof tcart__closeCart && (tcart__closeCart(), setTimeout((function() {
                    var e = t.closest(".t-popup_show");
                    if (e || (e = t.closest(".t706__cartwin_showed") || t.closest(".t706__cartwin")), e) {
                        ...
                        o && (o.style.display = "block"), a && (a.style.display = "block"), r && (r.style.display = "block"), i && (i.style.display = "block")
                    }
                }), 300))
            } catch (e) {}
            return !1
        }
        t_removeEventListener(n, "click", s), t_addEventListener(n, "click", s), t_addClass(o, "t-body_popupshowed");
        var d = document.getElementById("formbanktransfer"),
            c = [];

        function l(a) {
            if ((a = a || window.event).preventDefault ? a.preventDefault() : a.returnValue = !1, (c = window.tildaForm.validate(d)) && c.length > 0) return window.tildaForm.showErrors(d, c), !1;
            var i = t_serializeArray(d);
            i = t_forms__formData(i);
            var n = new XMLHttpRequest;
            n.open("POST", "https://" + window.tildaForm.endpoint + "/payment/banktransfer/", !0), n.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8"), n.onreadystatechange = function() {
                if (4 === n.readyState)
                    if (n.status >= 200 && n.status < 400) {
                        ...
                        if (a) {
                            var i = JSON.parse(a);
                            if ("object" == typeof i) {
                                if (t_removeClass(o, "t-body_popupshowed"), r && t_removeEl(r), i || (i = {
                                        error: "Unknown error. Please reload page and try again later."
                                    }), i && i.error) return alert(i.error), !1;
                                window.tildaForm.sendStatAndShowMessage(t, e, !0)
                            }
                        }
                    } else t_removeClass(o, "t-body_popupshowed"), r && t_removeEl(r), alert(i)
            }, n.send(i)
        }
        d && (t_removeEventListener(d, "submit", l), t_addEventListener(d, "submit", l))
